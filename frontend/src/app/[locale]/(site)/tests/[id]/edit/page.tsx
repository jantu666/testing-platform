  ...
  useEffect(() => {
    ...
    apiFetch<TestDetail>(`/tests/${id}`)
      .then((data) => {
        if (cancelled) return;
        const can =
          user.roles.includes('ADMIN') || data.creatorId === user.id;
        if (!can) {
          router.replace('/');
          return;
        }
  ...
  return (
    <div className="space-y-6">
      <div className="forum-card p-6 space-y-4">
        <h1 className="text-xl font-semibold text-white">{t('edit')}</h1>
        <div>
      ...
      <button type="button" onClick={addQuestion} className="text-forum-accent hover:underline">
        + {t('addQuestion')}
      </button>
      <div className="flex flex-wrap gap-3">
        <button
          ...
          className="rounded border border-forum-border px-4 py-2 text-white disabled:opacity-50"
        >
          {t('saveDraft')}
        </button>
        <button
